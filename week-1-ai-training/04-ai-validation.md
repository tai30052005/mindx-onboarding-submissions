# How Testing Controls AI-Generated Implementation

> **Deliverable 4.** This is the file most directly tied to the point of week 1.

## Questions this file answers

- How do tests give control over code an AI wrote?
- What changes when the test is written *before* the AI writes the implementation,
  compared to the other way round?
- What is the problem with letting the AI write both the tests and the code?
- What do I check before accepting AI-generated code, and in what order?

## Tests as an executable specification

> _Cần trả lời: vì sao "có phép kiểm chạy được" khác về bản chất với "đọc code rồi tin
> vào mắt mình"._

## Test-first vs code-first when working with AI

| | Test written first | Code generated first |
|---|---|---|
| What anchors the behaviour | | |
| What can silently go wrong | | |
| How a mistake surfaces | | |

## Why not let the AI write both the tests and the code

> _Cần trả lời: nêu rõ vòng lặp tự xác nhận ở đây. Câu này rất dễ bị hỏi._

## My review procedure before accepting AI output

> _Cần trả lời: dựa trên 5 câu hỏi ở `slides-ai-training.md` — Understanding, Alignment,
> Assumptions, Risks, Verification. Trình bày thành quy trình mình thật sự dùng, kèm ví dụ
> lấy từ `ai-workflow-log.md`._

## How I verified this

| Claim | How I checked it |
|---|---|
|  |  |

## Still unsure about

-
